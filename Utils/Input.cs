...
namespace DC_SB.Utils
{
    public class Input
    {
        ...
        public delegate void KeyboardHookCallback(VKeys key, List<VKeys> pressedKeys);

        public event KeyboardHookCallback KeyDown;
        public event KeyboardHookCallback KeyUp;
        public event KeyboardHookCallback KeyPress;

        private Dictionary<VKeys, VKeys> numpadKeys = new Dictionary<VKeys, VKeys>()
        {
            { VKeys.INSERT, VKeys.NUMPAD0 },
            { VKeys.DELETE, VKeys.DECIMAL },
            { VKeys.END, VKeys.NUMPAD1 },
            { VKeys.DOWN, VKeys.NUMPAD2 },
            { VKeys.PAGE_DOWN, VKeys.NUMPAD3 },
            { VKeys.LEFT, VKeys.NUMPAD4 },
            { VKeys.CLEAR, VKeys.NUMPAD5 },
            { VKeys.RIGHT, VKeys.NUMPAD6 },
            { VKeys.HOME, VKeys.NUMPAD7 },
            { VKeys.UP, VKeys.NUMPAD8 },
            { VKeys.PAGE_UP, VKeys.NUMPAD9 }
        };
        private bool shiftNumpadCorrection;
        private Timer shiftNumpadTimer = new Timer();
        private RawInput rawInput;

        private List<VKeys> pressedKeys = new List<VKeys>();
        ...
        {
            var key = (VKeys)e.KeyPressEvent.VKey;
            if (e.KeyPressEvent.Message == 256 || e.KeyPressEvent.Message == 260)
            {
                if (!(key == VKeys.SHIFT && pressedKeys.Contains(key)))
                {
        ...
        private void Key_Down(VKeys key)
        {
